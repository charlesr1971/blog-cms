...
<cfcomponent extends="taffy.core.resource" taffy_uri="/pages/dates/{year}/{month}/{usertoken}">
  ...
  <cffunction name="get">
    ...
    <cfset local.data['year'] = arguments.year>
    <cfset local.data['month'] = arguments.month>
    ...
    <cfif Len(Trim(local.data['usertoken']))>
      ...
      <cfif local.qGetUserID.RecordCount>
        <CFQUERY NAME="local.qGetFiles" DATASOURCE="#request.domain_dsn#">
          SELECT * 
          FROM tblFile 
          WHERE YEAR(Submission_date) = #arguments.year# AND MONTH(Submission_date) = #arguments.month# AND (Approved = <cfqueryparam cfsqltype="cf_sql_tinyint" value="1"><cfif Val(local.qGetUserID.User_ID)> OR (Approved = <cfqueryparam cfsqltype="cf_sql_tinyint" value="0"> AND User_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#local.qGetUserID.User_ID#">)</cfif>)
    ...
    <cfelse>
      <CFQUERY NAME="local.qGetFiles" DATASOURCE="#request.domain_dsn#">
          SELECT * 
          FROM tblFile 
          WHERE YEAR(Submission_date) = #arguments.year# AND MONTH(Submission_date) = #arguments.month# AND Approved = <cfqueryparam cfsqltype="cf_sql_tinyint" value="1">
          ORDER BY Submission_date DESC
        </CFQUERY>
      <cfif local.qGetFiles.RecordCount>
        <cfset local.data['pages'] = Ceiling(local.qGetFiles.RecordCount/request.filebatch)>
